...
export const makeMessagesSocket = (config: SocketConfig) => {
	...
	// Extended message handlers (album, event, pollResult, payment, groupStatus)
	const extHandlers = makeExtendedMessageHandlers({
		waUploadToServer,
		relayMessage,
		meJid: authState.creds.me?.id,
	})
	...
	return {
		...
		sendMessage: async (jid: string, content: ExtendedAnyMessageContent, options: MiscMessageGenerationOptions = {}) => {
			...
			if (
				...
				// Dispatch extended message types before the normal Baileys flow
				const extType = detectExtendedMessageType(content as object)
				if (extType) {
					switch (extType) {
						case 'ALBUM':
							return extHandlers.handleAlbum(content as unknown as AlbumMessageContent, jid, options)
						case 'EVENT':
							return extHandlers.handleEvent(content as unknown as EventV2MessageContent, jid, options)
						case 'POLL_RESULT':
							return extHandlers.handlePollResult(content as unknown as PollResultMessageContent, jid, options)
						case 'PAYMENT':
				...
				// Normalize AI message: strip the `ai` flag before handing to generateWAMessage
				// ({ ai: true, text: "..." } → { text: "..." })
				const isAiContent = 'ai' in content && !!(content as { ai?: boolean }).ai
				const normalizedContent: AnyMessageContent = isAiContent
					? { text: (content as { text: string }).text }
					: content as AnyMessageContent

				const fullMsg = await generateWAMessage(jid, normalizedContent, {
					logger,
					userJid,
					getUrlInfo: text =>
						getUrlInfo(text, {
				...
				const isEventMsg = 'event' in normalizedContent && !!(normalizedContent as any).event
				const isDeleteMsg = 'delete' in normalizedContent && !!(normalizedContent as any).delete
				const isEditMsg = 'edit' in normalizedContent && !!(normalizedContent as any).edit
				const isPinMsg = 'pin' in normalizedContent && !!(normalizedContent as any).pin
				const isPollMessage = 'poll' in normalizedContent && !!(normalizedContent as any).poll
				const additionalAttributes: BinaryNodeAttributes = {}
				const additionalNodes: BinaryNode[] = []
				// required for delete
				if (isDeleteMsg) {
					// if the chat is a group, and I am not the author, then delete the message as an admin
					if (isJidGroup((normalizedContent as any).delete?.remoteJid as string) && !(normalizedContent as any).delete?.fromMe) {
				...
				// AI flag — tells WA to render this message with AI styling
				if (isAiContent) {
					additionalNodes.push({ tag: 'bot', attrs: { biz_bot: '1' } } as BinaryNode)
				}
