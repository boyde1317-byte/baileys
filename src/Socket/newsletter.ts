...
export const makeNewsletterSocket = (config: SocketConfig) => {
	...
	return {
		...
		/**
		 * Resolve a newsletter invite URL to its display name and JID.
		 *
		 * @example
		 * const info = await sock.newsletterId('https://whatsapp.com/channel/abc123...')
		 * // { name: 'My Channel', id: '12345@newsletter' }
		 */
		newsletterId: async (url: string): Promise<{ name: string; id: string } | null> => {
			...
			return { name: metadata.name, id: metadata.id }
		},
	}
}
