...
import { areJidsSameUser, getBinaryNodeChild, getBinaryNodeChildren, getBizBinaryNode, isHostedLidUser, isHostedPnUser, isJidBot, isJidGroup, isJidMetaAI, isJidNewsletter, isLidUser, isPnUser, jidDecode, jidEncode, jidNormalizedUser, PSA_WID, S_WHATSAPP_NET, STORIES_JID } from '../WABinary/index.js';
...
export const makeMessagesSocket = (config) => {
    ...
    const getUSyncDevices = async (jids, useCache, ignoreZeroDevices) => {
        ...
        if (result) {
            // Store LID↔PN mappings from device query results
    ...
    return {
        ...
        readMessages,
        refreshMediaConn,
        // Arrow function (not a getter property) so that spreading this object in chats.ts
        // keeps the live closure binding to `mediaHost` rather than capturing a stale snapshot.
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            const userLid = authState.creds.me.lid || undefined;
        ...
        /**
         * Sends a status mention (story) to specific contacts or groups.
         * Supports text (with font/backgroundColor/textColor), image, video, and audio.
         *
         * @param {object} content - Message content (same as sendMessage)
         * @param {string[]} jids - Array of contact/group JIDs to mention
         * @param {object} [options] - Additional options (delayMs, font, textColor, backgroundColor, ptt)
         * @returns {Promise<object>} The sent message
         */
        sendStatusMention: async (content, jids = [], options = {}) => {
            const userJid = authState.creds.me.id;
            const allUsers = new Set();
            allUsers.add(jidNormalizedUser(userJid));
            const { delayMs = 1500 } = options;
            ...
            for (const id of jids) {
                if (isJidGroup(id)) {
                    try {
                        const groupData = (cachedGroupMetadata ? await cachedGroupMetadata(id) : null) || await groupMetadata(id);
                        for (const participant of groupData.participants) {
                            allUsers.add(jidNormalizedUser(participant.id));
                        }
                    } catch (error) {
            ...
            // For media (non-audio): convert text to caption
            if (isMedia && !isAudio) {
            ...
            // Random font and colors for text statuses
            ...
            const backgroundColor = (!isMedia || isAudio)
                ...
                : undefined;
            const ptt = isAudio
                ? (typeof content.ptt === 'boolean' ? content.ptt : true)
                : undefined;
            ...
            // Send status mention notifications to each JID
            for (const id of jids) {
                try {
                    ...
                    const mentionMsg = generateWAMessageFromContent(normalizedId, {
                        messageContextInfo: {
                            messageSecret: randomBytes(32)
                        },
                        [type]: {
                    ...
                    await relayMessage(normalizedId, mentionMsg.message, {
                        additionalNodes: [
                            {
                                tag: 'meta',
                                attrs: isPrivate
        ...
        /**
         * Sends a group status (story) to a specific group.
         * Uses groupStatusMessageV2 protocol.
         *
        ...
        sendGroupStatus: async (jid, content, options = {}) => {
            ...
            const waMsgContent = await generateWAMessageContent(storyContent, {
                logger,
                userJid,
                upload: waUploadToServer,
                mediaCache: config.mediaCache,
            ...
            // Find the message key and inject contextInfo
            ...
            if (msgKey) {
                ...
                // Set statusSourceType based on content type
                if (!innerMsg[msgKey].contextInfo.statusSourceType) {
                    if (innerMsg[msgKey].imageMessage) {
                        innerMsg[msgKey].contextInfo.statusSourceType = 0;
                    } else if (innerMsg[msgKey].videoMessage) {
                        innerMsg[msgKey].contextInfo.statusSourceType = 1;
                    } else if (innerMsg[msgKey].audioMessage) {
                        innerMsg[msgKey].contextInfo.statusSourceType = 3;
                    } else if (innerMsg[msgKey].extendedTextMessage) {
            ...
            await relayMessage(jid, finalMsg, {
                messageId: generateMessageIDV2(userJid)
            });
