...
/**
 * Extracts the invite code from a WhatsApp channel link.
 * Supports formats:
 *   https://whatsapp.com/channel/0029Vb7eSHf42Dcmdd3XA326
 *   whatsapp.com/channel/0029Vb7eSHf42Dcmdd3XA326
 *   0029Vb7eSHf42Dcmdd3XA326  (raw invite code)
 * Returns null if the input doesn't look like a channel link/invite.
 */
const extractNewsletterInviteCode = (input) => {
    if (!input || typeof input !== 'string')
        return null;
    const trimmed = input.trim();
    // Raw invite code (alphanumeric, typically ~22 chars, no slashes/colons)
    if (!trimmed.includes('/') && !trimmed.includes(':') && /^[A-Za-z0-9]{15,}$/.test(trimmed)) {
        return trimmed;
    }
    // Channel URL: extract the part after /channel/
...
export { extractNewsletterInviteCode };
export const makeNewsletterSocket = (config) => {
    ...
    return {
        ...
        executeWMexQuery,
        /**
         * Creates a WhatsApp newsletter/channel.
         *
         * @param {string} name - Channel display name (required)
         * @param {string} [description] - Channel description (optional)
         * @param {object} [options] - Additional options
         * @param {Buffer|Stream|string} [options.picture] - Channel profile picture
         *   (Buffer, readable stream, or file path). If provided, the image is
         *   processed and sent as a base64 JPEG in the create mutation.
         * @returns {Promise<object>} Parsed newsletter metadata from the create response
         */
        newsletterCreate: async (name, description, options = {}) => {
            const input = {
                name,
                description: description ?? null
            };
            // If a picture is provided, process it and include in the create mutation
            if (options.picture) {
                try {
                    const { img } = await generateProfilePicture(options.picture);
                    input.picture = img.toString('base64');
                }
                catch (err) {
                    console.warn('[Newsletter] Failed to process picture for create, continuing without:', err.message || err);
                }
            }
        ...
        /**
         * Resolves a channel link or invite code to a newsletter JID.
         *
         * Accepts:
         *   - Full channel URL: https://whatsapp.com/channel/0029Vb7eSHf42Dcmdd3XA326
         *   - Raw invite code: 0029Vb7eSHf42Dcmdd3XA326
         *   - Newsletter JID: 123456789@newsletter (returns as-is)
         *
         * @param {string} input - Channel link, invite code, or JID
         * @returns {Promise<{jid: string, metadata: object}|null>}
         */
        newsletterResolve: async (input) => {
            if (!input || typeof input !== 'string')
                return null;
            const trimmed = input.trim();
            // Already a JID
            if (trimmed.endsWith('@newsletter')) {
                const metadata = await this?.newsletterMetadata?.('jid', trimmed) ??
                    (await executeWMexQuery({
                        fetch_creation_time: true,
                        fetch_full_image: true,
                        fetch_viewer_metadata: true,
                        input: { key: trimmed, type: 'JID' }
                    }, QueryIds.METADATA, XWAPaths.xwa2_newsletter_metadata));
                return { jid: trimmed, metadata };
            }
            // Try to extract invite code from link or raw code
            const inviteCode = extractNewsletterInviteCode(trimmed);
            if (inviteCode) {
                const metadata = await executeWMexQuery({
                    fetch_creation_time: true,
                    fetch_full_image: true,
                    fetch_viewer_metadata: true,
                    input: { key: inviteCode, type: 'INVITE' }
                }, QueryIds.METADATA, XWAPaths.xwa2_newsletter_metadata);
                const parsed = parseNewsletterMetadata(metadata);
                if (parsed && parsed.id) {
                    return { jid: parsed.id, metadata: parsed };
                }
            }
            return null;
        },
