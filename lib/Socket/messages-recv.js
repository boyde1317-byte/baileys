...
export const makeMessagesRecvSocket = (config) => {
        ...
        switch (child?.tag) {
            ...
            case 'leave':
                ...
                const participants = getBinaryNodeChildren(child, 'participant').map(({ attrs }) => {
                    const pn = isLidUser(attrs.jid) && isPnUser(attrs.phone_number) ? attrs.phone_number : undefined;
                    const lid = isPnUser(attrs.jid) && isLidUser(attrs.lid) ? attrs.lid : undefined;
                    // Store LID↔PN mapping when both are available
                    if (pn && lid && signalRepository?.lidMapping) {
                        signalRepository.lidMapping.storeLIDPNMappings([{ lid, pn }]).catch(() => {});
                    }
                    return {
                        id: attrs.jid,
                        phoneNumber: pn,
            ...
            case 'revoked_membership_requests':
                ...
                // LID/PN mapping resolved via actingParticipantLid/affectedParticipantLid
                msg.messageStubType = WAMessageStubType.GROUP_MEMBERSHIP_JOIN_APPROVAL_REQUEST_NON_ADMIN_ADD;
                msg.messageStubParameters = [
                    JSON.stringify({ lid: affectedParticipantLid, pn: affectedParticipantPn }),
    ...
    const sendMessagesAgain = async (key, ids, retryNode, receiptNode) => {
        ...
        // True when the retry comes from the primary device (not a multi-device companion).
        // In that case we re-send to all devices to recover from first-message decryption failure.
        const isPrimaryDeviceRetry = !jidDecode(participant)?.device;
        ...
        logger.debug({ participant, isPrimaryDeviceRetry, shouldRecreateSession, recreateReason, injectedFromBundle }, 'prepared session for retry resend');
    ...
    const processNodeWithBuffer = async (node, identifier, handler) => {
        ev.buffer();
        await runBuffered();
        ev.flush();
        function runBuffered() {
            return handler(node, false).catch(err => onUnexpectedError(err, identifier));
        }
    };
