...
export const getBizBinaryNode = (message) => {
    ...
    // Carousel: nativeFlow buttons live inside each card, not at the top level.
    // Extract the first button name from the first card that has one.
    const carouselCards = message.interactiveMessage?.carouselMessage?.cards;
    const firstButtonName = flowMsg?.buttons?.[0]?.name ||
        carouselCards?.find(c => c.nativeFlowMessage)?.nativeFlowMessage?.buttons?.[0]?.name;
    const hasCarouselNativeFlow = !!carouselCards?.some(c => c.nativeFlowMessage);
    ...
    if (flowMsg || hasCarouselNativeFlow || message.buttonsMessage || message.templateMessage) {
        return {
            tag: 'biz',
            attrs: bizAttributes,
            content: [
