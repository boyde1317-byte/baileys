...
const baileysVersion = [2, 3000, 1044479778];
...
export const BufferJSON = {
    ...
    replacer: (k, value) => {
        if (Buffer.isBuffer(value) || value instanceof Uint8Array || value?.type === 'Buffer') {
            // Serialise binary data as a tagged base64 string for JSON transport
    ...
    reviver: (_, value) => {
        if (typeof value === 'object' && value !== null && value.type === 'Buffer' && typeof value.data === 'string') {
            // Deserialise a tagged base64 string back into a Node.js Buffer
...
export const debouncedTimeout = (intervalMs = 1000, task) => {
    ...
    return {
        ...
        // Updates the debounce delay for future invocations (does not restart any pending timeout)
        updateInterval: (newInterval) => (intervalMs = newInterval)
    };
};
...
export const fetchLatestBaileysVersion = async (options = {}) => {
    const URL = 'https://raw.githubusercontent.com/boyde1317-byte/baileys/main/lib/Defaults/index.js';
    try {
...
export const getCallStatusFromNode = ({ tag, attrs }) => {
    ...
    switch (tag) {
        ...
        case 'transport':
            status = 'transport';
            break;
        // 'relaylatency' is a WhatsApp telemetry status label, not a relay to an external server
        case 'relaylatency':
            status = 'relaylatency';
            break;
        case 'reject':
