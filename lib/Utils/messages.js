...
import { CALL_AUDIO_PREFIX, CALL_VIDEO_PREFIX, LIBRARY_NAME, MEDIA_KEYS, URL_REGEX, WA_DEFAULT_EPHEMERAL } from '../Defaults/index.js';
...
// Native flow CTA names verified against current WhatsApp clients.
// Names not listed here are EXPERIMENTAL: unverified CTAs can get the whole
// message rejected or render dead buttons. Opt in per-message with
// `experimentalCta: true` (or per-button with `experimental: true`).
const KNOWN_NATIVE_FLOW_CTAS = new Set([
    'quick_reply',
    'cta_copy',
    'cta_url',
    'cta_call',
    'single_select',
    'cta_request_location',
    'cta_request_phone',
    'send_location',
    'flow',
    'cta_reminder',
...
const assertCtaAllowed = (name, button, message) => {
    if (KNOWN_NATIVE_FLOW_CTAS.has(name)) {
        return;
    }
    const allowed = message?.experimentalCta === true || button.experimental === true;
    if (!allowed) {
        throw new Boom(`"${name}" is an experimental native flow CTA that is not verified against current WhatsApp clients. ` +
            `Enable it with { experimentalCta: true } on the message or { experimental: true } on the button, ` +
            `or use a verified CTA: ${[...KNOWN_NATIVE_FLOW_CTAS].join(', ')}`, { statusCode: 400 });
    }
};
/**
 * Maps a button definition to a native flow { name, params } pair.
 * Single source of truth shared by prepareNativeFlowButtons()
 * (nativeFlow / interactiveMessage) and the legacy `buttons`
 * (buttonsMessage) path so both cannot drift apart.
 * Returns null when the button is not a native flow button.
 * `raw: true` marks an explicit name/paramsJson passthrough (never gated).
 */
const mapButtonToNativeFlow = (button, { mapIdAsQuickReply = true } = {}) => {
    const buttonText = button.text || button.buttonText || button.display_text || button.label;
    const buttonIcon = button.icon?.toUpperCase();
    if (mapIdAsQuickReply && hasOptionalProperty(button, 'id') && !!button.id) {
        return { name: 'quick_reply', params: { display_text: buttonText || '👉🏻 Click', id: button.id, icon: buttonIcon } };
    }
    else if (hasOptionalProperty(button, 'copy') && !!button.copy) {
        return { name: 'cta_copy', params: { display_text: buttonText || '📋 Copy', copy_code: button.copy, icon: buttonIcon } };
    }
    else if (hasOptionalProperty(button, 'url') && !!button.url) {
        return { name: 'cta_url', params: { display_text: buttonText || '🌐 Visit', url: button.url, merchant_url: button.url, webview_interaction: button.useWebview, icon: buttonIcon } };
    }
    else if (hasOptionalProperty(button, 'call') && !!button.call) {
        return { name: 'cta_call', params: { display_text: buttonText || '📞 Call', phone_number: button.call, icon: buttonIcon } };
    }
    else if (hasOptionalProperty(button, 'sections') && !!button.sections) {
        return { name: 'single_select', params: { title: buttonText || '📋 Select', sections: button.sections, icon: buttonIcon } };
    }
    else if (hasOptionalProperty(button, 'location') && !!button.location) {
        return { name: 'cta_request_location', params: { display_text: buttonText || '📍 Share Location', icon: buttonIcon } };
    }
    else if (hasOptionalProperty(button, 'phone') && !!button.phone) {
        return { name: 'cta_request_phone', params: { display_text: buttonText || '📱 Share Phone', icon: buttonIcon } };
    }
    else if (hasOptionalProperty(button, 'sendLocation') && !!button.sendLocation) {
        return { name: 'send_location', params: { display_text: buttonText || '📍 Send Location', icon: buttonIcon } };
    }
    else if (hasOptionalProperty(button, 'flow') && !!button.flow) {
        const flow = typeof button.flow === 'string' ? { token: button.flow } : button.flow;
        return {
            name: 'flow',
            params: {
                display_text: buttonText || '📝 Open Form',
                flow_token: flow.token || '',
                flow_id: flow.id || flow.flowId || '',
                flow_cta_text: flow.ctaText || buttonText || '📝 Open Form',
                flow_action: flow.action || 'navigate',
                flow_action_payload: flow.actionPayload || flow.payload || {},
                flow_action_callback: flow.callback || '',
                flow_message_version: flow.version || 3,
                flow_expires_timestamp: flow.expiresAt || 0,
                flow_x_token: flow.xToken || '',
                icon: buttonIcon
            }
        };
    }
    else if (hasOptionalProperty(button, 'subscribe') && !!button.subscribe) {
        const sub = typeof button.subscribe === 'string' ? { id: button.subscribe } : button.subscribe;
        return {
            name: 'cta_subscribe',
            params: {
                display_text: buttonText || '🔔 Subscribe',
                subscription_id: sub.id || sub.subscriptionId || '',
                subscription_expiration: sub.expiration || sub.expiresAt || 0,
                icon: buttonIcon
            }
        };
    }
    else if (hasOptionalProperty(button, 'address') && !!button.address) {
        // NIXCODE 4.5 ships this as 'address_message' (proven in the wild);
        // 'cta_address' is not a verified name
        return {
            name: 'address_message',
            params: {
                display_text: buttonText || '📍 Share Address',
                id: typeof button.address === 'string' ? button.address : button.addressId || '',
                title: button.addressTitle || 'Address',
                description: button.addressDescription || '',
                form: button.addressForm || {},
                icon: buttonIcon
            }
        };
    }
    else if (hasOptionalProperty(button, 'signIn') && !!button.signIn) {
        const si = typeof button.signIn === 'string' ? { token: button.signIn } : button.signIn;
        return { name: 'cta_sign_in', params: { display_text: buttonText || '🔐 Sign In', sign_in_token: si.token || '', sign_in_expires_timestamp: si.expiresAt || 0, icon: buttonIcon } };
    }
    else if (hasOptionalProperty(button, 'signContract') && !!button.signContract) {
        const sc = typeof button.signContract === 'string' ? { token: button.signContract } : button.signContract;
        return { name: 'cta_sign_contract', params: { display_text: buttonText || '✍️ Sign Contract', contract_token: sc.token || '', contract_expires_timestamp: sc.expiresAt || 0, icon: buttonIcon } };
    }
    else if (hasOptionalProperty(button, 'completePayment') && !!button.completePayment) {
        const cp = typeof button.completePayment === 'string' ? { token: button.completePayment } : button.completePayment;
        return { name: 'cta_complete_payment', params: { display_text: buttonText || '💳 Complete Payment', payment_token: cp.token || '', payment_reference: cp.reference || '', icon: buttonIcon } };
    }
    else if (hasOptionalProperty(button, 'reviewAndPay') && !!button.reviewAndPay) {
        const rp = typeof button.reviewAndPay === 'string' ? { token: button.reviewAndPay } : button.reviewAndPay;
        return { name: 'cta_review_and_pay', params: { display_text: buttonText || '🧾 Review & Pay', order_id: rp.orderId || '', token: rp.token || '', payment_reference: rp.reference || '', icon: buttonIcon } };
    }
    else if (hasOptionalProperty(button, 'signUp') && !!button.signUp) {
        const su = typeof button.signUp === 'string' ? { token: button.signUp } : button.signUp;
        return { name: 'cta_sign_up', params: { display_text: buttonText || '📝 Sign Up', sign_up_token: su.token || '', sign_up_expires_timestamp: su.expiresAt || 0, icon: buttonIcon } };
    }
    else if (hasOptionalProperty(button, 'reminder') && !!button.reminder) {
        const rm = typeof button.reminder === 'string' ? { id: button.reminder } : button.reminder;
        return { name: 'cta_reminder', params: { display_text: buttonText || '⏰ Set Reminder', reminder_id: rm.id || '', reminder_title: rm.title || '', reminder_time: rm.time || '', icon: buttonIcon } };
    }
    else if (hasOptionalProperty(button, 'cancelReminder') && !!button.cancelReminder) {
        // NIXCODE 4.5 addCancelReminder — params mirror addReminder (display_text, id)
        const cr = typeof button.cancelReminder === 'string' ? { id: button.cancelReminder } : button.cancelReminder;
        return { name: 'cta_cancel_reminder', params: { display_text: buttonText || '🔕 Cancel Reminder', id: cr.id || '', reminder_id: cr.id || '', icon: buttonIcon } };
    }
    else if (hasOptionalProperty(button, 'openChat') && !!button.openChat) {
        const oc = typeof button.openChat === 'string' ? { jid: button.openChat } : button.openChat;
        return { name: 'cta_open_chat', params: { display_text: buttonText || '💬 Open Chat', target_jid: oc.jid || '', message_text: oc.message || '', icon: buttonIcon } };
    }
    else if (hasOptionalProperty(button, 'schedule') && !!button.schedule) {
        const sch = typeof button.schedule === 'string' ? { title: button.schedule } : button.schedule;
        return { name: 'cta_schedule', params: { display_text: buttonText || '📅 Schedule', title: sch.title || '', description: sch.description || '', start_time: sch.startTime || '', end_time: sch.endTime || '', icon: buttonIcon } };
    }
    else if (hasOptionalProperty(button, 'copyAddress') && !!button.copyAddress) {
        return { name: 'cta_copy_address', params: { display_text: buttonText || '📋 Copy Address', address: button.copyAddress, icon: buttonIcon } };
    }
    else if (hasOptionalProperty(button, 'amazonLink') && !!button.amazonLink) {
        const az = typeof button.amazonLink === 'string' ? { url: button.amazonLink } : button.amazonLink;
        return { name: 'cta_amazon_link', params: { display_text: buttonText || '🛒 View on Amazon', url: az.url || '', icon: buttonIcon } };
    }
    else if (hasOptionalProperty(button, 'deleteMessage') && !!button.deleteMessage) {
        return {
            name: 'cta_delete_message',
            params: {
                display_text: buttonText || '🗑️ Delete Message',
                message_key: typeof button.deleteMessage === 'string' ? { id: button.deleteMessage } : button.deleteMessage,
                icon: buttonIcon
            }
        };
    }
    else if (hasOptionalProperty(button, 'payment') && !!button.payment) {
        const pm = typeof button.payment === 'string' ? { token: button.payment } : button.payment;
        return {
            name: 'cta_payment',
            params: { display_text: buttonText || '💳 Pay', payment_token: pm.token || '', payment_amount: pm.amount || '', payment_currency: pm.currency || '', payment_reference: pm.reference || '', icon: buttonIcon }
        };
    }
    else if (hasOptionalProperty(button, 'paymentVerification') && !!button.paymentVerification) {
        const pv = typeof button.paymentVerification === 'string' ? { token: button.paymentVerification } : button.paymentVerification;
        return { name: 'cta_payment_verification', params: { display_text: buttonText || '✅ Verify Payment', verification_token: pv.token || '', verification_reference: pv.reference || '', icon: buttonIcon } };
    }
    else if (hasOptionalProperty(button, 'targetCta') && !!button.targetCta) {
        const tc = typeof button.targetCta === 'string' ? { text: button.targetCta } : button.targetCta;
        return { name: 'target', params: { display_text: buttonText || tc.text || '👉 Action', target_text: tc.text || '', icon: buttonIcon } };
    }
    else if (hasOptionalProperty(button, 'name') && !!button.name) {
        // Explicit raw passthrough: caller provides name + paramsJson as-is (never gated).
        return { name: button.name, params: undefined, raw: true };
    }
    return null;
};
const nativeFlowParamsJson = (button, mapped) => {
    if (mapped.raw) {
        return typeof button.paramsJson === 'string' ? button.paramsJson : JSON.stringify(button.paramsJson || {});
    }
    return JSON.stringify(mapped.params);
};
const prepareNativeFlowButtons = (message) => {
    ...
    return {
        buttons: correctedField.map(button => {
            const mapped = mapButtonToNativeFlow(button);
            if (!mapped) {
                return button;
            }
            if (!mapped.raw) {
                assertCtaAllowed(mapped.name, button, message);
            }
            return {
                name: mapped.name,
                buttonParamsJson: nativeFlowParamsJson(button, mapped)
            };
        }),
...
// hasOptionalProperty is an alias of hasNonNullishProperty (identical logic).
// Kept for API compatibility — many call sites use this name.
...
export const generateWAMessageContent = async (message, options) => {
    ...
    if (hasNonNullishProperty(message, 'raw')) {
        ...
        // Generic keys (items/posts/products/suggested) can collide with
        // other content types, so they only route to the rich response
        // builder when the message explicitly opts in with `rich: true`.
        ((hasNonNullishProperty(message, 'items') ||
            hasNonNullishProperty(message, 'posts') ||
            hasNonNullishProperty(message, 'products') ||
            hasNonNullishProperty(message, 'suggested')) && message.rich === true)) {
    ...
    if (hasNonNullishProperty(message, 'buttons')) {
        const buttonsMessage = {
            buttons: message.buttons.map(button => {
                const buttonText = button.text || button.buttonText || button.display_text || button.label;
                // legacy buttons path: `id` falls through to the classic
                // buttonsMessage quick-reply below instead of native flow
                const mapped = mapButtonToNativeFlow(button, { mapIdAsQuickReply: false });
                if (!mapped) {
                    return {
                ...
                return {
                    nativeFlowInfo: {
                        name: mapped.name,
                        paramsJson: nativeFlowParamsJson(button, mapped)
                    },
        ...
        interactiveMessage.nativeFlowMessage = prepareNativeFlowButtons(message);
        ...
        if (hasOptionalProperty(message, 'bloksWidget') && !!message.bloksWidget) {
            interactiveMessage.bloksWidget = {
                uuid: message.bloksWidget.uuid || '',
                data: message.bloksWidget.data || '',
                type: message.bloksWidget.type || '',
                fallback: message.bloksWidget.fallback || ''
            };
        }
        if (hasOptionalProperty(message, 'text')) {
            interactiveMessage.body = { text: message.text };
            if (hasOptionalProperty(message, 'title') || hasOptionalProperty(message, 'subtitle') || hasNonNullishProperty(message, 'image') || hasNonNullishProperty(message, 'video')) {
                const headerMedia = await prepareWAMessageMedia(message, options).catch(() => ({}));
                const isValidHeader = hasValidInteractiveHeader(headerMedia);
                if (isValidHeader || hasOptionalProperty(message, 'title') || hasOptionalProperty(message, 'subtitle')) {
        ...
        const interactiveMessage = {
            carouselMessage: {
                cards: await Promise.all(message.cards.map(async (card) => {
                    ...
                    const carouselCard = {};
                    if (hasOptionalProperty(card, 'nativeFlow') && !!card.nativeFlow) {
                        carouselCard.nativeFlowMessage = prepareNativeFlowButtons(card);
                    }
                    else if (hasOptionalProperty(card, 'buttons') && !!card.buttons) {
                        carouselCard.nativeFlowMessage = prepareNativeFlowButtons({ nativeFlow: card.buttons });
                    }
                    if (hasOptionalProperty(card, 'text')) {
                        carouselCard.body = { text: card.text };
                        if (hasOptionalProperty(card, 'title') || hasOptionalProperty(card, 'subtitle') || hasNonNullishProperty(card, 'image') || hasNonNullishProperty(card, 'video') || hasNonNullishProperty(card, 'product')) {
                            if (isValidHeader || hasOptionalProperty(card, 'title') || hasOptionalProperty(card, 'subtitle')) {
                                carouselCard.header = {
                                    title: card.title || '',
                                    subtitle: card.subtitle || '',
                                    hasMediaAttachment: isValidHeader || undefined
                                };
                                if (isValidHeader) {
                        ...
                        if (hasOptionalProperty(card, 'caption')) {
                            ...
                            // Card has neither text nor caption — set empty body
                            // so WhatsApp doesn't reject/blank the card.
                            carouselCard.body = { text: card.title || '' };
                        }
                        if (hasOptionalProperty(card, 'thumbnail') && !!card.thumbnail) {
                            carouselCard.jpegThumbnail = card.thumbnail;
                        }
                ...
                carouselCardType: message.carouselCardType || CarouselCardType.UNKNOWN,
                messageVersion: 1
            }
        };
        if (hasOptionalProperty(message, 'bloksWidget') && !!message.bloksWidget) {
    ...
    if (hasOptionalProperty(message, 'externalAdReply') && !!message.externalAdReply) {
        ...
        // Support both `url` and `sourceUrl` as the source URL field
        const sourceUrl = content.url || content.sourceUrl;
        if (!sourceUrl || typeof sourceUrl !== 'string') {
            throw new Boom('externalAdReply requires a valid url or sourceUrl string', { statusCode: 400 });
        }
        // Thumbnail can be a Buffer (inline) or a URL string (thumbnailUrl)
        if ('thumbnail' in content && content.thumbnail !== undefined && !Buffer.isBuffer(content.thumbnail)) {
            throw new Boom('Thumbnail must be a Buffer or use thumbnailUrl for a URL string', { statusCode: 400 });
        }
        const externalAdReply = {
            ...
            mediaUrl: sourceUrl,
            renderLargerThumbnail: content.largeThumbnail ?? content.renderLargerThumbnail ?? false,
            sourceUrl: sourceUrl,
            title: content.title || ''
        };
        // Respect a provided thumbnailUrl; only synthesize from sourceUrl if none given
        if (content.thumbnailUrl && typeof content.thumbnailUrl === 'string') {
            externalAdReply.thumbnailUrl = content.thumbnailUrl;
        } else if (content.thumbnail) {
            externalAdReply.thumbnail = content.thumbnail;
            externalAdReply.thumbnailUrl = sourceUrl + '?update=' + Date.now();
        }
        // Support originalImageUrl for higher-res ad reply images
        if (content.originalImageUrl) {
            externalAdReply.originalImageUrl = content.originalImageUrl;
        }
        // Support showAdAttribution flag
        if (content.showAdAttribution !== undefined) {
            externalAdReply.showAdAttribution = content.showAdAttribution;
        }
    ...
    // Fallback for pre-built message types (e.g. requestPaymentMessage).
    // If no branch above matched and the message contains a property ending
    // with "Message", pass it through directly to the proto. This mirrors
    // getContentType() which also looks for keys ending with "Message".
    if (Object.keys(m).length === 0) {
        for (const key of Object.keys(message)) {
            if (key.endsWith('Message') && message[key] != null && key !== 'senderKeyDistributionMessage') {
                m = { [key]: message[key] };
                break;
            }
        }
