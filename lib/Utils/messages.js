...
import { isJidGroup, isJidNewsletter, isJidStatusBroadcast, jidNormalizedUser, isLidUser } from '../WABinary/index.js';
...
import { prepareRichResponseMessage } from './rich/index.js';
        ...
        buttons: correctedField.map(button => {
            ...
            if (hasOptionalProperty(button, 'id') && !!button.id) {
                ...
                return {
                    ...
                    buttonParamsJson: JSON.stringify({
                        display_text: buttonText || '📍 Share Location',
                        icon: buttonIcon
                    })
                };
            }
                ...
                return {
                    ...
                    buttonParamsJson: JSON.stringify({
                        display_text: buttonText || '📱 Share Phone',
                        icon: buttonIcon
                    })
                };
            }
                ...
                return {
                    name: 'send_location',
                    buttonParamsJson: JSON.stringify({
                        display_text: buttonText || '📍 Send Location',
                        icon: buttonIcon
                    })
                };
            }
            else if (hasOptionalProperty(button, 'flow') && !!button.flow) {
                const flow = typeof button.flow === 'string'
                    ? { token: button.flow }
                    : button.flow;
                return {
                    name: 'flow',
                ...
                const sub = typeof button.subscribe === 'string'
                    ? { id: button.subscribe }
                    : button.subscribe;
                return {
                    name: 'cta_subscribe',
                ...
                const si = typeof button.signIn === 'string'
                    ? { token: button.signIn }
                    : button.signIn;
                return {
                    name: 'cta_sign_in',
                    buttonParamsJson: JSON.stringify({
                        display_text: buttonText || '🔐 Sign In',
                        sign_in_token: si.token || '',
                        sign_in_expires_timestamp: si.expiresAt || 0,
                        icon: buttonIcon
                    })
                };
            }
            else if (hasOptionalProperty(button, 'signContract') && !!button.signContract) {
                const sc = typeof button.signContract === 'string'
                    ? { token: button.signContract }
                    : button.signContract;
                return {
                    name: 'cta_sign_contract',
                    buttonParamsJson: JSON.stringify({
                        display_text: buttonText || '✍️ Sign Contract',
                        contract_token: sc.token || '',
                        contract_expires_timestamp: sc.expiresAt || 0,
                        icon: buttonIcon
                    })
                };
            }
            else if (hasOptionalProperty(button, 'completePayment') && !!button.completePayment) {
                const cp = typeof button.completePayment === 'string'
                    ? { token: button.completePayment }
                    : button.completePayment;
                return {
                    name: 'cta_complete_payment',
                    buttonParamsJson: JSON.stringify({
                        display_text: buttonText || '💳 Complete Payment',
                        payment_token: cp.token || '',
                        payment_reference: cp.reference || '',
                        icon: buttonIcon
                    })
                };
            }
            else if (hasOptionalProperty(button, 'reviewAndPay') && !!button.reviewAndPay) {
                const rp = typeof button.reviewAndPay === 'string'
                    ? { token: button.reviewAndPay }
                    : button.reviewAndPay;
                return {
                    name: 'cta_review_and_pay',
                    buttonParamsJson: JSON.stringify({
                        display_text: buttonText || '🧾 Review & Pay',
                        order_id: rp.orderId || '',
                        token: rp.token || '',
                        payment_reference: rp.reference || '',
                        icon: buttonIcon
                    })
                };
            }
            else if (hasOptionalProperty(button, 'signUp') && !!button.signUp) {
                const su = typeof button.signUp === 'string'
                    ? { token: button.signUp }
                    : button.signUp;
                return {
                    name: 'cta_sign_up',
                    buttonParamsJson: JSON.stringify({
                        display_text: buttonText || '📝 Sign Up',
                        sign_up_token: su.token || '',
                        sign_up_expires_timestamp: su.expiresAt || 0,
                        icon: buttonIcon
                    })
                };
            }
            else if (hasOptionalProperty(button, 'reminder') && !!button.reminder) {
                const rm = typeof button.reminder === 'string'
                    ? { id: button.reminder }
                    : button.reminder;
                return {
                    name: 'cta_reminder',
                    buttonParamsJson: JSON.stringify({
                        display_text: buttonText || '⏰ Set Reminder',
                        reminder_id: rm.id || '',
                        reminder_title: rm.title || '',
                        reminder_time: rm.time || '',
                        icon: buttonIcon
                    })
                };
            }
            else if (hasOptionalProperty(button, 'openChat') && !!button.openChat) {
                const oc = typeof button.openChat === 'string'
                    ? { jid: button.openChat }
                    : button.openChat;
                return {
                    name: 'cta_open_chat',
                    buttonParamsJson: JSON.stringify({
                        display_text: buttonText || '💬 Open Chat',
                        target_jid: oc.jid || '',
                        message_text: oc.message || '',
                        icon: buttonIcon
                    })
                };
            }
            else if (hasOptionalProperty(button, 'schedule') && !!button.schedule) {
                const sch = typeof button.schedule === 'string'
                    ? { title: button.schedule }
                    : button.schedule;
                return {
                    name: 'cta_schedule',
                    buttonParamsJson: JSON.stringify({
                        display_text: buttonText || '📅 Schedule',
                        title: sch.title || '',
                ...
                return {
                    name: 'cta_copy_address',
                    buttonParamsJson: JSON.stringify({
                        display_text: buttonText || '📋 Copy Address',
                        address: button.copyAddress,
                        icon: buttonIcon
                    })
                };
            }
            else if (hasOptionalProperty(button, 'amazonLink') && !!button.amazonLink) {
                const az = typeof button.amazonLink === 'string'
                    ? { url: button.amazonLink }
                    : button.amazonLink;
                return {
                    name: 'cta_amazon_link',
                    buttonParamsJson: JSON.stringify({
                        display_text: buttonText || '🛒 View on Amazon',
                        url: az.url || '',
                        icon: buttonIcon
                    })
                };
                ...
                return {
                    ...
                    buttonParamsJson: JSON.stringify({
                        ...
                        message_key: typeof button.deleteMessage === 'string'
                            ? { id: button.deleteMessage }
                ...
                const pm = typeof button.payment === 'string'
                    ? { token: button.payment }
                    : button.payment;
                return {
                    name: 'cta_payment',
                    buttonParamsJson: JSON.stringify({
                        display_text: buttonText || '💳 Pay',
                        payment_token: pm.token || '',
                        payment_amount: pm.amount || '',
                        payment_currency: pm.currency || '',
                        payment_reference: pm.reference || '',
                        icon: buttonIcon
                    })
                };
            }
            else if (hasOptionalProperty(button, 'paymentVerification') && !!button.paymentVerification) {
                const pv = typeof button.paymentVerification === 'string'
                    ? { token: button.paymentVerification }
                    : button.paymentVerification;
                return {
                    name: 'cta_payment_verification',
                    buttonParamsJson: JSON.stringify({
                        display_text: buttonText || '✅ Verify Payment',
                        verification_token: pv.token || '',
                        verification_reference: pv.reference || '',
                        icon: buttonIcon
                    })
                };
            }
            else if (hasOptionalProperty(button, 'targetCta') && !!button.targetCta) {
                const tc = typeof button.targetCta === 'string'
                    ? { text: button.targetCta }
                    : button.targetCta;
                return {
                    name: 'target',
                    buttonParamsJson: JSON.stringify({
                        display_text: buttonText || tc.text || '👉 Action',
                        target_text: tc.text || '',
                        icon: buttonIcon
                    })
                };
                ...
                return {
                    name: button.name,
                    buttonParamsJson: typeof button.paramsJson === 'string'
                        ? button.paramsJson
...
export const generateWAMessageFromContent = (jid, message, options) => {
    ...
    const messageJSON = {
        ...
        participant: isJidGroup(jid) || isJidStatusBroadcast(jid) ? (options.userLid || userJid) : undefined,
