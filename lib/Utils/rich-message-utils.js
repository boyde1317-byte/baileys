...
export const prepareRichResponseMessage = (content) => {
    const { alignment, code, contentText, disclaimerText, dynamic, footerText, gridImage, headerText, imageText, inlineImage, inlineVideo, items, language, latex, links, noHeading, posts, products, quoted, suggested, richResponse, table, tapLinkUrl, text, tip, title } = content;
    let submessages = [];
    if (Array.isArray(richResponse)) {
        submessages = richResponse.map((submessage) => {
            ...
            throw new Error(`Unknown submessage type in richResponse array: ${JSON.stringify(Object.keys(submessage))}. Supported entries: text, code, items, inlineImage, inlineVideo, latex, gridImage, dynamic, posts, products, suggested, table, tip`);
        });
    }
    else {
        if (headerText) {
