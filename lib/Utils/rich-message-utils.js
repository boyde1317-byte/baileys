...
export const defaultRenderLatexToPng = async (latex, opts = {}) => {
    ...
    if (!_mathjaxInstance) {
        try {
            ...
            throw new Error(
                'defaultRenderLatexToPng requires mathjax-node (optional peer dep).\n' +
                'Either: npm install mathjax-node\n' +
                'Or: pass a custom renderLatexToPng(laatex, opts) => {buffer, width, height}\n' +
                '    to generateLatexImageContent / generateLatexInlineImageContent / generateLatexImageContentV2.\n' +
                'Alternative renderers: mathjax-full, katex + puppeteer, or any LaTeX-to-PNG library.'
            );
        }
    }
