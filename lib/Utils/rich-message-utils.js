 ...
 *
 * Fork note (boyde1317-byte/baileys), originally authored by itsliaaa (itsliaaa/Baileys):
 * - unifiedResponse.data is base64-encoded (matches NIXCODE and ourin-baileys convention)
 * - single AI_BOT_JID constant shared by the V1/V2 paths
 * - quoted/reply support in prepareRichResponseMessage
 * - additional submessage types (inline video, posts, products, grid image, dynamic, map, latex)
 */
...
const NOOP = new Set([]);
/** Bot JID used in forwardedAiBotMessageInfo for rich responses (both V1 and V2 paths). */
const AI_BOT_JID = '867051314767696@bot';
/**
 * Encodes a unified response payload for proto AIRichResponseMessage.unifiedResponse.data.
 * Base64-encoded, matching the convention used by NIXCODE and ourin-baileys.
 */
export const encodeUnifiedResponseData = (unified) => Buffer.from(JSON.stringify(unified)).toString('base64');
...
// ── Map tile (OSM) — no proven GenAIMapUXPrimitive exists (device audit
// 2026-09-12: renders caption-only). Maps render as a static OSM tile via the
// proven GenAIImaginePrimitive + a markdown info section. Requires attribution.
const osmTileUrl = (lat, lon, zoom = 16) => {
    const n = 2 ** zoom;
    const x = Math.floor(((lon + 180) / 360) * n);
    const latRad = (lat * Math.PI) / 180;
    const y = Math.floor(((1 - Math.log(Math.tan(latRad) + 1 / Math.cos(latRad)) / Math.PI) / 2) * n);
    return `https://tile.openstreetmap.org/${zoom}/${Math.min(Math.max(x, 0), n - 1)}/${Math.min(Math.max(y, 0), n - 1)}.png`;
};

// ── LaTeX — the proven shape is GenAILatexItem INLINE ENTITIES inside a
// GenAIMarkdownTextUXPrimitive, with {{KEY}}expr{{/KEY}} tags in the text
// (NIXCODE 4.5 anatomy, see Moonson lib/NIXCODE.js extractIE). The
// standalone GenAILatexUXPrimitive never painted on device (caption-only).
const latexTextSection = (text, expressions = []) => {
    let tagged = text || '';
    const inlineEntities = (expressions || []).map((expr, i) => {
        const key = `NIXEL_LATEX_${i}`;
        const tex = expr.latexExpression || expr.latex || '';
        ...
        return {
            ...
            metadata: {
                ...
                latex_image: {
                    url: expr.url || '',
                    width: Number(expr.width) || 200,
                    height: Number(expr.height) || 60,
                },
                font_height: Number(expr.fontHeight) || 83.333333333,
                padding: Number(expr.padding ?? expr.imageLeadingPadding) || 15,
...
const imagineVideoSection = (url, duration = 0, mime = 'video/mp4') => ({
    view_model: {
        primitive: {
            media: { url, mime_type: mime, duration },
...
// Map liaaa-family UX primitives to the device-verified NIXCODE GenAIImagine
// primitives. Device test 2026-09-12 (.testrich all, fork r4, real device):
// GenAIImageUX / GenAIGridImageUX / GenAIContentItemsUX / GenAIDynamicUX
// paint as empty gray boxes; GenAIImaginePrimitive (Moonson production shape)
// is the only proven image/video render. Opt-in via primitiveStyle: 'nixcode'.
const sectionsToNixcode = (sections) => sections.flatMap((section) => {
    const p = section?.view_model?.primitive;
    if (!p) return [section];
    const seen = new Set();
    const uniq = (urls) => urls.filter(Boolean).filter((u) => (seen.has(u) ? false : seen.add(u)));
    switch (p.__typename) {
        case 'GenAIImageUXPrimitive': {
            const url = p.image_url?.image_high_res_url || p.image_url?.image_preview_url || '';
            return url ? [imagineImageSection(url)] : [section];
        }
        case 'GenAIGridImageUXPrimitive': {
            const urls = uniq([
                p.grid_image_url?.image_high_res_url || p.grid_image_url?.image_preview_url || '',
                ...(p.image_urls || []).map((u) => u.image_high_res_url || u.image_preview_url || '')
            ]);
            return urls.length ? urls.map(imagineImageSection) : [section];
        }
        case 'GenAIContentItemsUXPrimitive': {
            const videos = uniq((p.items || []).map((it) => it?.reel_item?.video_url));
            return videos.length ? videos.map((u) => imagineVideoSection(u)) : [section];
        }
        case 'GenAIDynamicUXPrimitive': {
            if (!p.url) return [section];
            return [imagineVideoSection(p.url, 0, p.type === 'GIF' ? 'image/gif' : 'video/mp4')];
        }
        default:
            return [section];
    }
...
// NIXCODE 4.5 shapes — addImage/addVideo (GenAIImaginePrimitive),
// addProduct (GenAIProductItemCardPrimitive), addPost (GenAIPostPrimitive).
// Used only when primitiveStyle: 'nixcode' is set.
const normalizeProductCards = (products) => (Array.isArray(products) ? products : [products]).map((item) => ({
    title: item.title ?? item.name ?? '',
    brand: item.brand ?? '',
    price: item.price,
    sale_price: item.sale_price ?? item.salePrice ?? '',
    product_url: item.product_url ?? item.url ?? item.videoUrl ?? '',
    image: { url: item.image_url ?? item.image ?? item.imageUrl ?? item.thumbnail ?? item.thumbnailUrl ?? '' },
    additional_images: [{ url: item.icon_url ?? item.icon ?? item.profileIconUrl ?? '' }],
    __typename: 'GenAIProductItemCardPrimitive'
}));
const normalizePostCards = (posts, isCarousel) => (Array.isArray(posts) ? posts : [posts]).map((p) => ({
    title: p.title ?? '',
    subtitle: p.subtitle ?? '',
    username: p.username ?? p.author ?? '',
    profile_picture_url: p.profile_picture_url ?? p.profile_url ?? p.profileIconUrl ?? p.authorAvatar ?? '',
    is_verified: !!(p.is_verified || p.verified),
    thumbnail_url: p.thumbnail_url ?? p.thumbnail ?? p.thumbnailUrl ?? p.imageUrl ?? '',
    post_caption: p.post_caption ?? p.caption ?? '',
    likes_count: p.likes_count ?? p.like ?? 0,
    comments_count: p.comments_count ?? p.comment ?? 0,
    shares_count: p.shares_count ?? p.share ?? 0,
    post_url: p.post_url ?? p.url ?? p.videoUrl ?? '',
...
export const toUnified = (submessages, uuid, style = 'liaaa') => ({
    response_id: uuid || randomUUID(),
    sections: submessages.flatMap((submessage, index) => {
        switch (submessage.messageType) {
            ...
            case RichSubMessageType.CONTENT_ITEMS: {
                // primitiveStyle: 'nixcode' — emit NIXCODE 4.5's GenAI
                // Imagine / ProductItemCard / Post primitives instead of
                // the liaaa-family ContentItemsUX primitive
                if (style === 'nixcode' && submessage.imagineVideos?.length) {
                    return submessage.imagineVideos.map((v) => imagineVideoSection(v.url, v.duration));
                }
                if (style === 'nixcode' && submessage.productCards?.length) {
                    return submessage.productCards.length === 1
                        ? { view_model: { primitive: submessage.productCards[0], __typename: 'GenAISingleLayoutViewModel' } }
                        : { view_model: { primitives: submessage.productCards, __typename: 'GenAIHScrollLayoutViewModel' } };
                }
                if (style === 'nixcode' && submessage.postCards?.length) {
                    return { view_model: { primitives: submessage.postCards, __typename: 'GenAIHScrollLayoutViewModel' } };
                }
            ...
            case RichSubMessageType.INLINE_IMAGE: {
                ...
                if (style === 'nixcode') {
                    return imagineImageSection(imageUrl.imageHighResUrl || imageUrl.imagePreviewUrl || '');
                }
                return {
                    view_model: {
                        primitive: {
            ...
            case RichSubMessageType.LATEX: {
                // Proven shape (2026-09-14 fix): GenAILatexItem inline entities
                // inside a markdown text primitive — the NIXCODE/Moonson anatomy.
                const latexMeta = submessage.latexMetadata;
                return latexTextSection(latexMeta.text || '', latexMeta.expressions || []);
            }
            case RichSubMessageType.MAP: {
                // Proven shape (2026-09-14 fix): no GenAIMapUXPrimitive ever
                // rendered from bots (caption-only, audit 2026-09-12). Maps
                // paint as a static OSM tile (GenAIImaginePrimitive) + a
                // markdown info section for the annotations.
                const mapMeta = submessage.mapMetadata;
                const pinLat = (mapMeta.annotations && mapMeta.annotations[0]?.latitude) ?? mapMeta.centerLatitude;
                const pinLon = (mapMeta.annotations && mapMeta.annotations[0]?.longitude) ?? mapMeta.centerLongitude;
                const info = (mapMeta.annotations || []).map((a, i) =>
                    `${i === 0 ? '📍' : '•'} **${a.title || 'Location'}** — ${a.body || ''} _(${a.latitude}, ${a.longitude})_`
                ).join('\n');
                const mapText = (info ? info + '\n\n' : '') + `_[${mapMeta.centerLatitude}, ${mapMeta.centerLongitude}]_\n_Map data © OpenStreetMap contributors_`;
                return [
                    imagineImageSection(osmTileUrl(pinLat, pinLon)),
                    {
                        view_model: {
                            primitive: {
                                text: mapText,
            ...
            case RichSubMessageType.TABLE: {
                ...
                // Pad ragged rows to equal width (matches NIXCODE / ourin-baileys
                // behavior — uneven rows can render blank cells)
                const tableRows = tableMetadata.rows || [];
                const maxCells = tableRows.reduce((max, row) => Math.max(max, (row.items || []).length), 0);
                return {
                    view_model: {
                        primitive: {
                            title: tableMetadata.title,
                            rows: tableRows.map((row) => {
                                const cells = [...(row.items || []), ...Array(Math.max(0, maxCells - (row.items || []).length)).fill('')];
                                return {
                                    is_header: row.isHeading,
                                    cells,
                                    markdown_cells: cells.map((item) => ({ text: item }))
                                };
                            }),
            ...
            case RichSubMessageType.GRID_IMAGE: {
                ...
                // primitiveStyle: 'nixcode' — one GenAIImaginePrimitive section
                // per image (NIXCODE's addImage)
                if (style === 'nixcode') {
                    const urls = [
                        gridMeta?.gridImageUrl?.imagePreviewUrl || gridMeta?.gridImageUrl?.imageHighResUrl || '',
                        ...(gridMeta?.imageUrls || []).map((iu) => iu.imagePreviewUrl || iu.imageHighResUrl || '')
                    ].filter(Boolean);
                    const seen = new Set();
                    return urls
                        .filter((u) => (seen.has(u) ? false : seen.add(u)))
                        .map(imagineImageSection);
                }
                const toImgUrl = (iu) => ({
            ...
            case RichSubMessageType.DYNAMIC: {
                ...
                if (style === 'nixcode') {
                    return imagineVideoSection(dynMeta?.url || '', 0, dynType === 'GIF' ? 'image/gif' : 'video/mp4');
                }
                return {
                    view_model: {
                        primitive: {
            ...
            case RichSubMessageType.TEXT:
                // `isSuggestionPills` renders the follow-up prompts as tappable
                // pills (NIXCODE's addSuggest → GenAIFollowUpSuggestionPillPrimitive)
                if (submessage.isSuggestionPills) {
                    return {
                        view_model: {
                            primitives: (submessage.suggestions || []).map((pillText) => ({
                                prompt_text: pillText,
                ...
                // `isSourcesRow` renders the sources strip (NIXCODE's
                // addSource → GenAISearchResultPrimitive)
                if (submessage.isSourcesRow) {
                    return {
                        view_model: {
                            primitive: {
                ...
                // `isMetadataText` marks the small tip/hint line under the
                // content (NIXCODE's addTip → GenAIMetadataTextPrimitive)
                if (submessage.isMetadataText) {
                    return {
                        view_model: {
                            primitive: {
...
const normalizeSuggestionPills = (suggested) => {
    const arr = Array.isArray(suggested)
        ? suggested.map((v) => (typeof v === 'string' ? v : v.text || v.prompt || ''))
        : [typeof suggested === 'string' ? suggested : suggested.text || suggested.prompt || ''];
    return arr.filter(Boolean);
};
const normalizeSourcesRow = (sources) => {
    const raw = Array.isArray(sources) ? sources : [sources];
    // NIXCODE addSource format: flat string array = one [profile_url, url, text]
    // group; array of string arrays = multiple groups. Objects also accepted:
    // { profileUrl | favicon, url, title | text }
    const groups = raw.every((item) => typeof item === 'string')
        ? [raw]
        : raw.map((item) => Array.isArray(item) ? item : [
            item.profileUrl || item.favicon || '',
            item.url || '',
            item.title || item.text || ''
        ]);
    return groups.map(([profileUrl, url, text]) => ({
...
export const prepareRichResponseMessage = (content) => {
    const { alignment, code, contentText, disclaimerText, dynamic, footerText, gridImage, headerText, imageText, inlineImage, inlineVideo, items, language, latex, links, noHeading, posts, products, quoted, suggested, richResponse, table, tapLinkUrl, text, tip, title, sources, primitiveStyle } = content;
    let submessages = [];
    if (Array.isArray(richResponse)) {
        submessages = richResponse.map((submessage) => {
            if (submessage.text) {
                ...
                return {
                    ...
                    imagineVideos: [{ url: v.videoUrl || '', duration: v.duration ?? 0 }]
                };
            }
                ...
                const normalizeUrl = (u) => typeof u === 'string'
                    ...
                    : u;
                // NIXCODE addImage parity: a bare string array means
                // [main, ...others]
                const giArr = Array.isArray(gi);
                const mainUrl = giArr ? gi[0] : (gi.gridImageUrl || gi.mainImage || gi);
                const others = giArr ? gi.slice(1) : (gi.imageUrls || gi.images || []);
                return {
                    messageType: RichSubMessageType.GRID_IMAGE,
                    gridImageMetadata: {
                        gridImageUrl: normalizeUrl(mainUrl),
                        imageUrls: others.map(normalizeUrl),
                    }
                };
            }
                ...
                return {
                    ...
                    postCards: normalizePostCards(submessage.posts, (Array.isArray(submessage.posts) ? submessage.posts.length : 1) > 1)
                };
            }
                ...
                return {
                    ...
                    productCards: normalizeProductCards(submessage.products)
                };
            }
            else if (submessage.suggested) {
                const pills = normalizeSuggestionPills(submessage.suggested);
                return {
                    messageType: RichSubMessageType.TEXT,
                    messageText: pills.join('\n'),
                    suggestions: pills,
                    isSuggestionPills: true
                };
            }
            else if (submessage.sources) {
                const row = normalizeSourcesRow(submessage.sources);
                return {
                    messageType: RichSubMessageType.TEXT,
                    messageText: row.map((src) => src.source_display_name).join('\n'),
                    sources: row,
                    isSourcesRow: true
                };
            }
            else if (submessage.tip) {
                return {
                    messageType: RichSubMessageType.TEXT,
                    messageText: submessage.tip,
                    isMetadataText: true
                };
            }
            ...
            else if (submessage.messageType !== undefined && (submessage.messageText !== undefined || submessage.codeMetadata || submessage.tableMetadata || submessage.contentItemsMetadata || submessage.imageMetadata || submessage.latexMetadata || submessage.gridImageMetadata || submessage.dynamicMetadata || submessage.mapMetadata)) {
                // pre-formed submessage — passed through as-is (advanced usage)
                return submessage;
            }
            throw new Error(`Unknown submessage type in richResponse array: ${JSON.stringify(Object.keys(submessage))}. Supported entries: text, code, items, inlineImage, inlineVideo, latex, gridImage, dynamic, posts, products, suggested, sources, table, tip`);
        });
    }
    else {
        if (headerText) {
        ...
        if (code) {
            // `language` is destructured const — cannot be reassigned (fixes
            // TypeError when code is provided without a language)
        ...
        if (gridImage) {
            const normalizeUrl = (u) => typeof u === 'string'
                ? { imagePreviewUrl: u, imageHighResUrl: u, sourceUrl: '' }
                : u;
            const giArr = Array.isArray(gridImage);
            const mainUrl = giArr ? gridImage[0] : (gridImage.gridImageUrl || gridImage.mainImage || gridImage);
            const others = giArr ? gridImage.slice(1) : (gridImage.imageUrls || gridImage.images || []);
            submessages.push({
                messageType: RichSubMessageType.GRID_IMAGE,
                gridImageMetadata: {
                    gridImageUrl: normalizeUrl(mainUrl),
        ...
        if (posts) {
            submessages.push({
                ...
                postCards: normalizePostCards(posts, posts.length > 1)
            });
        }
        if (products) {
            submessages.push({
                ...
                productCards: normalizeProductCards(products)
            });
        }
        if (sources) {
            const row = normalizeSourcesRow(sources);
            submessages.push({
                messageType: RichSubMessageType.TEXT,
                messageText: row.map((src) => src.source_display_name).join('\n'),
                sources: row,
        ...
        if (suggested) {
            const pills = normalizeSuggestionPills(suggested);
            submessages.push({
                messageType: RichSubMessageType.TEXT,
                messageText: pills.join('\n'),
                suggestions: pills,
    ...
    const unified = toUnified(submessages, uuid, primitiveStyle === 'nixcode' ? 'nixcode' : 'liaaa');
    const richResponseMessage = proto.AIRichResponseMessage.create({
        submessages,
        messageType: proto.AIRichResponseMessageType.AI_RICH_RESPONSE_TYPE_STANDARD,
        unifiedResponse: {
            data: encodeUnifiedResponseData(unified)
        },
...
// Originally authored by ourin-baileys (npm: ourin-baileys). Included with attribution.
// ============================================================================
...
export const buildV2Content = (sections, ctxInfo, options = {}) => {
    const effective = options.primitiveStyle === 'nixcode' ? sectionsToNixcode(sections) : sections;
    return _buildV2ContentRaw(effective, ctxInfo);
};
const _buildV2ContentRaw = (sections, ctxInfo) => ({
    messageContextInfo: {
        threadId: [],
        deviceListMetadata: {
            senderKeyIndexes: [],
    ...
    botForwardedMessage: {
        message: {
            richResponseMessage: {
                submessages: [],
                messageType: proto.AIRichResponseMessageType.AI_RICH_RESPONSE_TYPE_STANDARD,
                unifiedResponse: { data: encodeUnifiedResponseData({ response_id: randomUUID(), sections }) },
                contextInfo: ctxInfo,
            },
        },
    },
...
// V1 generators: embed a base64 unifiedResponse alongside the submessage
// metadata (the shape Moonson/NIXCODE ships in production). Device-verified
// 2026-09-12: submessage metadata alone renders tables/code/text, but media
// (images/GIFs/reels) stays a gray box without unified view models. Also
// fixes captureUnifiedResponse, which requires unifiedResponse.data.
export const buildBotForwardedMessageUnified = (submessages, ctxInfo, options) => {
    const uuid = randomUUID();
    const style = options?.primitiveStyle === 'nixcode' ? 'nixcode' : 'liaaa';
    return buildBotForwardedMessage(submessages, ctxInfo, {
        data: encodeUnifiedResponseData(toUnified(submessages, uuid, style))
    });
};
...
export const generateTableContent = (title, headers, rows, quoted, options = {}) => {
    ...
    return {
        message: buildBotForwardedMessageUnified(submessages, ctxInfo, options),
...
export const generateTableContentV2 = (table, quoted, options = {}) => {
    ...
    return {
        message: buildV2Content(sections, ctxInfo, options),
        messageId: generateMessageIDV2(),
    };
};
...
export const generateLinkContent = (text, links, quoted, options = {}) => {
    ...
    const unifiedDataBytes = encodeUnifiedResponseData(unifiedData);
    const ctxInfo = {
        forwardingScore,
        isForwarded: true,
        forwardedAiBotMessageInfo: { botJid },
...
export const generateLatexContentV2 = (quoted, options = {}) => {
    ...
    // Proven shape (2026-09-14 fix): GenAILatexItem inline entities inside a
    // markdown text primitive — the NIXCODE/Moonson anatomy. The standalone
    // GenAILatexUXPrimitive never painted on device (caption-only).
    sections.push(latexTextSection(text, expressions));
    if (footer) {
        sections.push({
            view_model: {
                primitive: {
...
export const generateMapContentV2 = (params, quoted, options = {}) => {
    ...
    // Proven shape (2026-09-14 fix): static OSM tile (GenAIImaginePrimitive)
    // + markdown info section. GenAIMapUXPrimitive never rendered from bots.
    const pinLat = annotations[0]?.latitude ?? centerLatitude;
    const pinLon = annotations[0]?.longitude ?? centerLongitude;
    const info = annotations.map((a, i) =>
        `${i === 0 ? '📍' : '•'} **${a.title || 'Location'}** — ${a.body || ''} _(${a.latitude}, ${a.longitude})_`
    ).join('\n');
    sections.push(imagineImageSection(osmTileUrl(pinLat, pinLon)));
    sections.push({
        view_model: {
            primitive: {
                text: (info ? info + '\n\n' : '') + `_[${centerLatitude}, ${centerLongitude}]_\n_Map data © OpenStreetMap contributors_`,
                __typename: 'GenAIMarkdownTextUXPrimitive',
            },
            __typename: 'GenAISingleLayoutViewModel',
        },
...
export const generateLatexImageContentV2 = async (quoted, options, uploadFn, renderLatexToPng = defaultRenderLatexToPng) => {
    ...
    // Proven shape (2026-09-14 fix): GenAILatexItem inline entities inside a
    // markdown text primitive (NIXCODE/Moonson anatomy).
    sections.push(latexTextSection(text, latexExpressions));
    if (footer) {
        sections.push({
            view_model: {
                primitive: {
...
// ============================================================================
// NEW: Submessage combinations that render natively in WhatsApp
// These produce visible UI combinations that Meta AI uses in production
// ============================================================================

// --- V1: Code block + Table (code output results) ---

/**
 * Generates a richResponseMessage with a code block followed by a table.
 * Renders natively as: syntax-highlighted code → data table below it.
 *
 * @param {object}  params
 * @param {string}  params.code         — Code content
 * @param {string}  params.language     — Code language (default: javascript)
 * @param {string}  params.tableTitle   — Table title
 * @param {Array}   params.tableHeaders — ['Output', 'Value']
 * @param {Array}   params.tableRows    — [['stdout', 'hello world'], ...]
 * @param {object}  quoted
 * @param {object}  [options] — { headerText, footer }
 */
...
export const generateCodeWithTableV2 = (params, quoted, options = {}) => {
    ...
    const ctxInfo = buildV2ContextInfo(quoted);
    return { message: buildV2Content(sections, ctxInfo, options), messageId: generateMessageIDV2() };
};
...
export const generateMapWithTableV2 = (params, quoted, options = {}) => {
    ...
    // Proven shape (2026-09-14 fix): static OSM tile (GenAIImaginePrimitive)
    // + markdown info section. GenAIMapUXPrimitive never rendered from bots.
    const pinLat = (map.annotations && map.annotations[0]?.latitude) ?? map.centerLatitude;
    const pinLon = (map.annotations && map.annotations[0]?.longitude) ?? map.centerLongitude;
    const mapInfo = (map.annotations || []).map((a, i) =>
        `${i === 0 ? '📍' : '•'} **${a.title || 'Location'}** — ${a.body || ''} _(${a.latitude}, ${a.longitude})_`
    ).join('\n');
    sections.push(imagineImageSection(osmTileUrl(pinLat, pinLon)));
    sections.push({
        view_model: {
            primitive: {
                text: (mapInfo ? mapInfo + '\n\n' : '') + `_[${map.centerLatitude}, ${map.centerLongitude}]_\n_Map data © OpenStreetMap contributors_`,
                __typename: 'GenAIMarkdownTextUXPrimitive',
            },
            __typename: 'GenAISingleLayoutViewModel',
        },
